...
def surprisal(c1, c2, normalize=False):
    """
    ...
    surprisal = total_comb * H(c_comb) - total_c1*H(c1) - total_c2*H(c2)

    if surprisal < 0:
        print "Warning! Negative Surprisal value! Defaulting to 0!"
        surprisal = 0.0

    if (normalize and total_comb != 0):
        surprisal /= float(total_comb)

    return surprisal


def multi_surprisal(*args, **kwargs):
    """
    ...
    surprisal = total_comb * H(c_comb)
    for i in range(len(args)):
        surprisal -= totals[i]*H(args[i])

    # normalize the surprisal, if specified
    if (normalize and total_comb != 0):
        surprisal /= float(total_comb)
