...
def plot_landscape(TPTdir, unfolded_state=0):
        ...
        while (i<(toppaths.shape[1]-1)) and (toppaths[n,i+1] >= 0):
            ...
            if not UseNewArrows:
                ...
                plt.arrow( pfold[j], F[j], 0.95*(pfold[k]-pfold[j]), 0.95*(F[k]-F[j]), fc=arrowcolor, ec=arrowcolor,
                       head_width=0.015, head_length=0.01, width=0.005*c.convert(totalflux[(j,k)]) )
            plt.hold(True)
            i += 1

    plt.xlabel('$p_{fold}$')
    plt.ylabel('free energy (kcal/mol)')
    plt.axis( [-0.1, 1.1, -2.5, 3.5] )
...
if __name__ == '__main__':
    ...
    # Create the figure
    f = Figure(plt)
    f.add_panel(0.05, 0.10, 0.45, 0.8)      # [xorig, yorig, width, height]
    f.add_panel(0.55, 0.10, 0.45, 0.8)
   
    ################################
    # Panel A 
    ax = plt.axes(f.axis_handles[0])
    #plot_landscape('TPT-wt-macro20', unfolded_state=11)
    plot_landscape('TPT-wt-macro100', unfolded_state=10)#64)

    ###############################
    # Panel B 
    ax = plt.axes(f.axis_handles[1])
    #plot_landscape('TPT-tz4-macro20', unfolded_state=11)
    plot_landscape('TPT-tz4-macro100', unfolded_state=64)


#outfile = 'tpt.eps'
outfile = 'tpt-labels-100.eps'
#outfile = 'tpt-100.eps'
#outfile = 'tpt-20.eps'
plt.show()
