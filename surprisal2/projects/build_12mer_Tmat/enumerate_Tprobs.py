...
# REQUIRES HPSandbox library to build models
import sys
...
>> Try:  enumerate_Tprobs.py mc12.conf

This program will read in an HP chain specified in the configuration file,
and perform a full enumeration of conformational space. 

For each microstate, the probability of MC moves to other states are evaluated
to construct a transition probabilty matrix (the move set is specified in the config file)

NOTE: movesets are described in Dill et al. Protein Science 4: 561-602, 1995:
    MS1: (i) three-bead flip, (ii) end flips
    MS2: (i) three-bead flip, (ii) end flips, (iii) crankshaft moves, and (iv) rigid rotations

Values printed as output:
...
FILES written as output:
    microT.mtx       - 15037 x 15037 sparse matrix for microstate Tprob    
    microstates.dat  - a description of each microstate 
...
if __name__ == '__main__':
    ...
    if os.path.exists(microTFn):
      ...
      for i in range(len(microstates)):
        ...
        # make a list of possible transitions to states j
        jumps = [i]

        # self-transitions are always possible
        C[i,i] = 1.0
        rowcount = 1.0
        ...
        # row-normalize so each microstate gets an equal number of counts
        # make sure all self-transition probs are >= 0.5 by adding double self counts
        C[i,i] += rowcount
        for j in jumps:
            C[i,j] = C[i,j]/(2*rowcount)

        # make sure all self-transition probs are >= 0.5
        

      # Compute the (naive) transition matrix from counts 
      if (1):
          #T = estimate_transition_matrix(C)
          C_sym, T, populations, mapping = build_msm(C, symmetrize='MLE', ergodic_trimming=False)
          print 'C_sym, T, populations, mapping', C_sym, T, populations, mapping
