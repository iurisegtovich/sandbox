...
if __name__ == '__main__':
  ...
  if (1):
    ...
    print 'Reading in transition matrices...'   #tCounts.mtx.RER		tCounts.mtx.RRR
    C_wt = np.array(mmread('tCounts.mtx.RRR').todense()) 
    C_mut = np.array(mmread('tCounts.mtx.RER').todense()) 
    ...
    # Create the figure
    f = Figure(plt, figsize=(4, 3), linewidth=2.0, markersize=5,
                       fontsize=14.0, legend_fontsize='medium', xlabelsize = 'small', ylabelsize='small' )
    f.add_panel(0.60, 0.15, 0.38, 0.82)     # [xorig, yorig, width, height]
  ...
  if (1):
    ...
    D_values = [10**i for i in np.arange(-8,0,0.5)]
    for D in D_values:
...
outfile = 'fspeptide_RRR_RER_jsds.eps'
