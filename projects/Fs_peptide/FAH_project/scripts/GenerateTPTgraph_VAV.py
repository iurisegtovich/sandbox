...
#with some default parameters (EdgeScale=1, PopCutoff=0.01, EdgeCutoff=0.1) in the run() function.\n\n""")
    ...
    # must be old version of the parser --VAV
    ...
    popFn = 'Data-combined-macro40/Populations.dat'
    tmat = 'Data-combined-macro40/net_Fluxes.mtx'  #'net_flux.mtx'
    epsilon = 0.000
    directed = True #False
    input_dir = None
    outFn = 'Graph-combined-macro40-netflux.dot'

    matrix = io.mmread(tmat)
    populations = np.loadtxt(popFn, dtype=float)
    
    run(matrix, populations, input_dir, Directed=directed, OutputFile=outFn, PopCutoff=epsilon)
