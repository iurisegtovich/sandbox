...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    if (npes!=NPES){
    printf("Only accept %d processors",NPES);
    exit(0);
    }
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        if (my_PE_num==0){
        MPI_Send(&Temperature[ROWS][0],COLUMNS+2,MPI_DOUBLE,my_PE_num + 1,DOWN, MPI_COMM_WORLD);
        MPI_Recv(&Temperature[ROWS+1][0],COLUMNS+2,MPI_DOUBLE,my_PE_num + 1,UP,MPI_COMM_WORLD,&status);
        }
        else if (my_PE_num==3){
                MPI_Recv(&Temperature[0][0],COLUMNS+2,MPI_DOUBLE,my_PE_num - 1,DOWN,MPI_COMM_WORLD,&status);
                MPI_Send(&Temperature[1][0],COLUMNS+2,MPI_DOUBLE,my_PE_num - 1,UP, MPI_COMM_WORLD);
             }
        else{
        MPI_Recv(&Temperature[0][0],COLUMNS+2,MPI_DOUBLE,my_PE_num - 1,DOWN,MPI_COMM_WORLD,&status);
        MPI_Send(&Temperature[1][0],COLUMNS+2,MPI_DOUBLE,my_PE_num - 1,UP, MPI_COMM_WORLD);
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        //if (my_PE_num==2) printf("dt_global: %f.\n",dt_global);
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    for (i = 0; i <= ROWS+1; i++) {
        ...
        //if (my_PE_num == 2){
        //    printf("\nPE:%d. %f,\n",my_PE_num,Temperature_last[i][COLUMNS+1]);
        //}
